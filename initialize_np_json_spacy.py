"""
Based on script initialize_np_json_parzu.py,  started by Jay Suchardt, Niels Lange, Johanna Wrede in summer semester 2023 in the course "Korpuslinguistische Analyse der Nominalflextion im Deutschen"
edited by Ronja Laarmann-Quante

Modified to apply to spacy parses:
wordtype N -> wordtype NOUN or PROPN
deprel kon -> deprel cj


Last update: 2023_08_24
"""
...
def add_must_agree_relations(np_dict: dict) -> dict:
    ...
    for token1 in np_dict["TOK_info"]:
        for token2 in np_dict["TOK_info"]:
            ...
            if np_dict["TOK_info"][token1]["word_type"]  in ["NOUN", "PROPN"] and np_dict["TOK_info"][token2]["word_type"] in ["DET", "ADJ"]:
            ...
            if np_dict["TOK_info"][token1]["word_type"] in ["DET", "ADJ"] and np_dict["TOK_info"][token2]["word_type"] in ["DET", "ADJ"] and token1 != token2:
        ...
        for token2 in np_dict["HEAD_info"]["head"]:
            if np_dict["TOK_info"][token1]["word_type"] in ["NOUN", "PROPN", "DET", "ADJ"] and np_dict["HEAD_info"]["head"][token2]["POS_STTS"] == "APPRART":

                rel_dict = build_relations_dict()
                rel_dict["rel_type"] = "must_agree"
                rel_dict["involved"] = [token1, token2]
                rel_dict["ruler"] = None
                if np_dict["TOK_info"][token1]["word_type"]  in ["NOUN", "PROPN"]: rel_dict["ruler"] = token1
                np_dict["relations"]["rel_" + str(rel_counter)] = rel_dict
                rel_counter += 1
                all_relations.append({token1, token2})
...
def run_script(infile, outfile, corpusname):
    ...
    # read output file
    with open (infile, mode="r", encoding="utf-8-sig") as f:  #utf-8-sig to avoid BOM error
